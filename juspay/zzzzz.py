...
class Tree:
    ...
    def lock(self,node,uid):
        if node.isLocked or node.isDescendantLocked>0 or self.isAncestorsLocked(node.parent):
            return False
        node.uid=uid
        node.isLocked=True
    ...
    def upgrade(self,node,uid):
        if node.isDescendantLocked==0 or self.isAncestorsLocked(node.parent):
            return False
        self.unlockChildren(node,uid)
        self.lock(node,uid)

        return True
    def isAncestorsLocked(self,node):
        while node:
            if node.isLocked:
                return True
            node=node.parent
